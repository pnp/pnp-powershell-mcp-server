...
- Added `data/script-samples.json` and `data/pnp-commands.json` as embedded resources, generated by `build/Update-VendoredData.ps1`, so the three script-sample tools answer offline instead of erroring unless an unrelated VS Code extension happens to be installed.
- Added the `pnp_get_result_page` tool: a JSON result set over the output cap is summarised with its true row count, field names and as many whole rows as fit, then paged from the session that produced it rather than by re-running the command.
- Added record-and-playback testing through `PNP_MCP_RECORD_DIR` and `PNP_MCP_REPLAY_DIR`, with `TranscriptScrubber` removing tenant hostnames, identities, GUIDs, secrets, thumbprints, certificates and profile-path account names before a fixture is written.
- Added `ToolSelectionEvaluator` and `e2eTestPrompts.md`, a BM25 gate over the published tool descriptions requiring every prompt to rank its tool in the top three; the baseline is 56/56 top-3 and 89 % at or above the 0.4 confidence target.
...
- Added a vendored-index fallback to `pnp_search_commands`, so it still names cmdlets and their documentation where `pwsh` or `PnP.PowerShell` is missing.
- Added `StdioProtocolTests`, which drives the built server as a real process over JSON-RPC so the published wire format is tested rather than the SDK talking to itself.
- Added a bare-runner CI job that takes `pwsh` off PATH before running the suite, which is the only place the cold-start states and the offline claims are exercised for real.
- Added tests for scrubber fuzzing, summarising and paging, the vendored indexes, fixture-key stability, and BM25 agreement with a model reading the same descriptions (93 %).

### Changed

- Changed every tool description, driven by what the evaluator measured: `pnp_run_command` described its mechanism rather than its job and no task-shaped prompt selected it at all.
- Changed `ScriptSampleTools` to read from `ScriptSampleIndex`, which treats the VS Code extension and `PNP_SCRIPT_SAMPLES_PATH` as overrides ahead of the vendored index rather than as the only sources.
